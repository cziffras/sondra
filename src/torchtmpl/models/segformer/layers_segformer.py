...
class MixMLP(nn.Sequential):
    ...
    def __init__(self, channels: int, expansion: int = 4):
        super().__init__(
            nn.Conv2d(channels, channels, kernel_size=1, dtype=torch.complex64),
            nn.Conv2d(
                channels,
                channels * expansion,
                kernel_size=3,
            ...
            CGELU(),
            nn.Conv2d(channels * expansion, channels, kernel_size=1, dtype=torch.complex64),
        )
...
class SegFormerSegmentationHead(nn.Module):
    def __init__(self, channels: int, num_classes: int, num_features: int = 4):
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(
                channels * num_features, channels, kernel_size=1, bias=False, dtype=torch.complex64
            ),
            modReLU(),
