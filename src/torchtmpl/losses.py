...
class NTXentKendall(nn.Module):
    ...
    def __init__(
        self, num_stages: int, temperature: float = 0.1, eps: float = 1e-6, lambda_reg: float = 1.0
    ):
        super().__init__()
...
class NTXentKLUnif(nn.Module):
    ...
    def __init__(
        self, num_stages: int, temperature: float = 0.1, eps: float = 1e-6, lambda_reg: float = 1
    ):
        super().__init__()
        self.nt_xent = NTXentLoss(temperature, eps)
...
class NTXentLoss(nn.Module):
    ...
    def forward(
        self, z1: torch.Tensor, z2: torch.Tensor, temperature: float | torch.Tensor | None = None
    ) -> torch.Tensor:
        """
        Args:
...
class FocalLoss(nn.Module):
    def __init__(
        self,
        alpha=torch.tensor(
            [2.38, 40.0, 3.03, 27.77, 10.27, 15.24, 55.0]
        ),  # torch.tensor([1.0, 1.46, 0.11, 1.19, 0.40, 0.54, 2.29]
        ignore_index=0,
        gamma=1.2,
    ):
