...
def visualize_complex_img(img, normalize=True):
    ...
    return {"magnitude": magnitude, "phase": phase, "real": real, "imag": imag}
        ...
        if channels:
            for c in range(C):
                try:
                    ...
                    print(f"Error in visualization example {i + 1}, channel {c}: {e!s}")
        else:
            try:
                view1, view2 = aug(img_tensor)
                ...
                print(f"Error in visualization example {i + 1}: {e!s}")
