...
class FourierAugment:
    ...
    def __init__(
        self, mask_prob=0.1, structured_mask=True, preserve_center=True, center_radius=0.1
    ):
    ...
    def __call__(self, img):
        ...
        for c in range(C):
            ...
            if self.preserve_center:
                ...
                dist = (y - center_h) ** 2 / (radius_h**2) + (x - center_w) ** 2 / (radius_w**2)
                center_mask = (dist <= 1).to(torch.float32)
    ...
    def _create_structured_mask(self, H, W, device):
        ...
        if mask_type == "lines":
            ...
            for i in range(num_sectors):
                ...
                sector_mask = (angles >= start_angle) & (angles < start_angle + sector_width)
                mask[sector_mask] = 0
...
class SpeckleAugment:
    ...
    def __call__(self, img):
        ...
        if self.distribution == "gamma":
            k = 1.0 / self.intensity
            noise = (
                torch.from_numpy(np.random.gamma(k, scale=1.0 / k, size=(C, H, W)))
                .to(img.device)
                .to(img.real.dtype)
            )
        else:
...
class SARContrastiveAugmentations:
    ...
    def __init__(
        ...
        self.rotate_flip = RotateFlip(rotate_prob=rotate_flip_prob, flip_prob=rotate_flip_prob)
